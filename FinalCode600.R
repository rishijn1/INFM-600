...
#this eliminates the ones less than 30
df3 <- subset(df2, df2$langknown > 30)
df3
#use xyplot to plot the new dataframe
library(lattice)
xyplot(df3$langknown~df3$country, xlab ="Country", ylab = "Average Number of Languages
       Known")
#question 4
...
class(data2$JobSearchStatus)
data2$JobSearchStatus<-  ifelse(data2$JobSearchStatus=="I am actively looking for a job",1,
                           ifelse(data2$JobSearchStatus=="I am not interested in new job opportunities",2,
                                  ifelse(data2$JobSearchStatus=="I am not actively looking, but I am open to new opportunities",3,NA)))

class(data2$JobSearchStatus)
cor.test(data2$YearsCodingProf,data2$JobSearchStatus)
#question5
#turns the categorocal variables into numeric
...
boxplot(data2$LastNewJob,data2$ConvertedSalary)
...
plot(data2$StackOverflowJobs, las= 2)
summary(data2$StackOverflowJobs)
